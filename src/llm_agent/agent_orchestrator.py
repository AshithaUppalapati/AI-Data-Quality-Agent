...
def save_report(report: dict) -> str:
    ...
    serializable = {k: v for k, v in report.items() if k != "spark"}
...
def run_dq_agent(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "%d anomalies detected | Health: %d/100 (%s)",
                anomaly_report["total_anomalies"],
                anomaly_report["health_score"],
                anomaly_report["health_status"]
            )
        except Exception as e:
        ...
        logger.info(
            "Combined health: %d/100 (%s)",
            combined_report["health_score"], combined_report["health_status"]
        )
    ...
    logger.info(
        "Status: %s | Health: %s/100 | Cost: $%s",
        report["status"].upper(),
        report.get("health_score", "N/A"),
        report.get("total_cost_usd", 0)
    )
